...
internal static class Dumper
{
    ...
    private static readonly string OutDir =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "FMAnalyzer");
    ...
    {
        // Atomair: eerst naar data.json.tmp schrijven en pas na een geslaagde Close
        // over data.json heen schuiven. De web-app kan data.json op elk moment lezen;
        ...
        string path = Path.Combine(OutDir, "data.json");
        string tmp = path + ".tmp";
        ...
        Dictionary<int, int> hist, long candidates, long ms)
    {
        try
        {
            ...
            w.WriteLine($"FMAnalyzer diagnostics — {DateTime.Now}");
